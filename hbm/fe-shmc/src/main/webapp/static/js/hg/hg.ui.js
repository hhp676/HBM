...
(function(){
	...
	HgUi.ok = function(msg, callback) {
        ...
        okMsg.append("<div style='float: left;padding-left: 5px;width:220px;margin-top: 15px;'>" + msg+ "</div>");
		$.messager.alert({
			title:"提示",
			msg:okMsg.toString(),
			icon:null,
        ...
        $('.messager-window .panel-tool-close').click(callback);
	};
	
	HgUi.alert = function(msg, callback) {
		var alertMsg = new StringBuffer();
		alertMsg.append("<div class='messager-icon messager-warning' style='float: left;'></div>");
		alertMsg.append("<div style='float: left;padding-left: 5px;width:220px;margin-top: 15px;'>" + msg+ "</div>");
		$.messager.alert({
			title:"提示",
	...
	HgUi.error = function(msg, callback) {
		var okMsg = new StringBuffer();
		okMsg.append("<div class='messager-icon messager-error style='float: left;'></div>");
        okMsg.append("<div style='float: left;padding-left: 5px;width:220px;margin-top: 15px;'>" + msg+ "</div>");
		$.messager.alert({
			title:"错误",
			msg:okMsg.toString(),
	...
	HgUi.notice = function (msg, callback) {
        var okMsg = new StringBuffer();
        okMsg.append("<div class='messager-icon messager-warning style='float: left;'></div>");
        okMsg.append("<div style='float: left;padding-left: 5px;width:220px;margin-top: 15px;'>" + msg+ "</div>");
		$.messager.alert({
			title:"通知",
			msg:okMsg.toString(),
					...
					if (me.isEditing) {
						if (me.editingIndex != index) {
							...
							if (me.options.saveBtn.length > 0) {
								$.messager.confirm("确认保存", "<br>是否保存编辑状态的数据？", function(flag){
									if (flag) {
