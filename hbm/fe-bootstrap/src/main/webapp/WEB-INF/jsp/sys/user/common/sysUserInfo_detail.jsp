	...
	<div class="easyui-layout" data-options="fit:true" id="sysUserInfoDetail_layout">
		<h:hasAnyPermissions name='<%=Auths.Sys.USER_UPDATE + "," + Auths.Sys.USER_DELETE%>'>
               ...
               <h:hasAllPermissions name="<%=Auths.Sys.USER_UPDATE%>">
                   <a class="btn btn-success btn-sm" iconCls="icon_warning"  plain="true"
                       onclick="sysUserInfoDetail.toEdit()">修改</a>
               </h:hasAllPermissions>
               <h:hasAllPermissions name="<%=Auths.Sys.USER_DELETE%>">
                   <a class="btn btn-darkorange btn-sm" iconCls="icon_delete"  plain="true"
					...
					<form class="hgform">
						<table class="hgtable" style="line-height: 25px">
							<tr>
								...
								<td align="right" style="width: 60px;">英文名：</td>
								<td style="width: 170px;">
							...
							<tr>
								...
								<td colspan="3">
									<textarea name="descr" cols=80 rows=3 disabled="disabled" style="width: 374px; height: 30px;">${user.descr}</textarea><br/>
								</td>
							</tr>
						</table>
						<input type="hidden" id="isDelete" value="${user.isDelete}">
					</form>
				</div>
