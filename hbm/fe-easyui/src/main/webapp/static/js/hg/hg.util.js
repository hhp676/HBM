...
(function(){
    ...
    HgUtil.getDictComboboxData = function(groupCode){
        ...
        for(var key=0;key<itemMap.length;key++){
            comboboxData.push({
    ...
    if (typeof document.cancelFullScreen != "undefined") {
    	HgUtil.fullScreen.supports = true;
    } else {
        for (var i = 0; i < HgUtil.fullScreen.prefixes.length; i++) {
        	HgUtil.fullScreen.prefix = HgUtil.fullScreen.prefixes[i];
            if (typeof document[HgUtil.fullScreen.prefix + "CancelFullScreen"] != "undefined") {
            	HgUtil.fullScreen.supports = true;
                break;
            }
        }
    ...
    if (HgUtil.fullScreen.supports) {
    	HgUtil.fullScreen.eventName = HgUtil.fullScreen.prefix + "fullscreenchange";
    	HgUtil.fullScreen.isFullScreen = function () {
            switch (this.prefix) {
        ...
        HgUtil.fullScreen.requestFullScreen = function(elem) {
            return (this.prefix === "") ? elem.requestFullScreen() : elem[this.prefix + "RequestFullScreen"]();
        };
        HgUtil.fullScreen.cancelFullScreen = function (elem) {
            return (this.prefix === "") ? document.cancelFullScreen() : document[this.prefix + "CancelFullScreen"]();
        };
    }
    
    HgUtil.isFullScreen = function(){
    	return HgUtil.fullScreen.isFullScreen();
    };
    
    HgUtil.requestFullScreen = function (selector) {
        if (selector == null || selector == undefined) { selector = document.documentElement; }
        selector = $(selector);
        return selector.each(function () {
            if (HgUtil.fullScreen.supports) {HgUtil.fullScreen.requestFullScreen(this); }
        });
    };
    HgUtil.cancelFullScreen = function (selector) {
        if (selector == null || selector == undefined) { selector = document.documentElement; }
        selector = $(selector);
        return selector.each(function () {
            if (HgUtil.fullScreen.supports) {HgUtil.fullScreen.cancelFullScreen(this); }
        });
    };
